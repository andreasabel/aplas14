...
⟦∗⟧ {a = a} {b∞ = b∞}  (↿ (▹ 𝒕)) (↿ (▹_ {t = u} 𝒕₁)) 
 =  ↿ exp β▹
     (▹ ≡.subst (λ t → SAT.satSet (⟦ force b∞ ⟧ _) (app t u))
          renId (out≤′ (force b∞) (≤⇒≤′ ≤ℕ.refl) (𝒕 _ ≤ℕ.refl id (in≤′ a (≤⇒≤′ ≤ℕ.refl) 𝒕₁)))) 
⟦∗⟧ {a = a} {b∞ = b∞}  (↿ (▹ 𝒕)) (↿ ne 𝒏) = ↿ ne (elim 𝒏 (∗r (▹ SAT.satSN (⟦ a ⟧≤ ⟦→⟧ ⟦ force b∞ ⟧≤) 𝒕)))
⟦∗⟧ (↿ (▹ 𝒕))    (↿ exp t⇒ 𝑡) = ↿ exp (cong (∗r _) (∗r _) t⇒) (⇃ ⟦∗⟧  (↿ (▹ 𝒕)) (↿ 𝑡))
...
sound (var x)   θ = θ x
sound {n} (abs {a = a} {b = b} t) {σ = σ} θ = ⟦abs⟧ {𝓐 = ⟦ _ ⟧≤} {𝓑 = ⟦ _ ⟧≤} (λ l≤m ρ {u} 𝑢 → 
  let open ≡-Reasoning
      ...
      eq = begin
           ...
           ∎
  in (≡.subst (λ tu → tu ∈⟨ l≤m ⟩ (⟦ b ⟧≤)) eq (↿ in≤′ b (≤⇒≤′ l≤m) (⇃ sound t (Ext (↿ out≤′ a (≤⇒≤′ l≤m) (⇃ 𝑢)) ((Rename ρ (Map l≤m θ))))))))

sound {n} (app {a} {b} t t₁)  θ = ↿ out≤′ b (≤⇒≤′ ≤ℕ.refl) 
       (⇃ ⟦app⟧ {n} {𝓐 = ⟦ _ ⟧≤} {𝓑 = ⟦ _ ⟧≤} ≤ℕ.refl (sound t θ) (↿ in≤′ a (≤⇒≤′ ≤ℕ.refl) (⇃ sound t₁ θ) ))
sound (pair t t₁) θ = ⟦pair⟧ (sound t θ) (sound t₁ θ)
sound (fst t)     θ = ⟦fst⟧ {𝓐 = ⟦ _ ⟧ _} {𝓑 = ⟦ _ ⟧ _} (sound t θ)
sound (snd t)     θ = ⟦snd⟧ {𝓐 = ⟦ _ ⟧ _} {𝓑 = ⟦ _ ⟧ _} (sound t θ)
sound (t ∗ t₁)    θ = ⟦∗⟧ (sound t θ) (sound t₁ θ)
sound {zero} (▹ t) θ = ↿ ▹0
